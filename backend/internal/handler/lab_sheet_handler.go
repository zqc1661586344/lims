...
func (h *LabSheetHandler) CreateTemplate(c *gin.Context) {
	var req struct {
		...
		Structure      []byte `json:"structure" binding:"required"`
		EditableRanges []byte `json:"editable_ranges"`
		ReadOnlyRanges []byte `json:"readonly_ranges"`
	...
	if len(tpl.EditableRanges) == 0 {
		tpl.EditableRanges = []byte("[]")
	}
	if len(tpl.ReadOnlyRanges) == 0 {
		tpl.ReadOnlyRanges = []byte("[]")
	}
